...
class BacktestMexc:
    ...
    def get_time_range_custome(self,
        days_ago: int,
        ...
        - days_ago : jumlah hari ke belakang (berbasis kalender)
        - end_time :
            - None               -> sekarang (WIB)
            - datetime           -> waktu akhir eksplisit
            - str 'YYYY-MM-DD'   -> akhir hari (23:59:59 WIB)
        """

        wib = pytz.timezone('Asia/Jakarta')

        # 1. Tentukan end_datetime
        if end_time is None:
            end_datetime = datetime.now(wib)

        elif isinstance(end_time, str):
            # Format: '2025-12-10' → akhir hari
            end_datetime = wib.localize(
            ...
            if end_time.tzinfo is None:
                end_datetime = wib.localize(end_time)
            else:
                end_datetime = end_time.astimezone(wib)

        else:
            raise TypeError("end_time harus None, datetime, atau str 'YYYY-MM-DD'")

        # 2. Timestamp akhir
        end = int(end_datetime.timestamp() * 1000)

        # 3. Timestamp awal (berbasis kalender)
